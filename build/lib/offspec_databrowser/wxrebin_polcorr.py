...
class rebinned_data():
  ...
  def setThetaOffset(self, offset):
    self.__theta_offset = offset
    self.make_2th_th_map()
    if self.qxqz:
      self.runConversion(self.qxqz.params)
  ...
  def make_2th_th_map_polcorr(self, evt = None, grid = None):
    ...
    for i in range(num_files):
      ...
      pol_corr_full_array = zeros((pol_corr_list.shape[0], len(intens_correction), 4)) 
