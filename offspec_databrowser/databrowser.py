...
from offspec_databrowser.wxrebin_polcorr import rebinned_data
from offspec_databrowser.binned_data_class3 import plottable_2d_data
from offspec_databrowser.he3analyzer import He3Analyzer 
...
class DataBrowser(wx.Frame):
    ...
    def __init__(self, data_directory = None, parent = None, load_saved_objects = True):
        ...
        self.dataplot = self.fig.axes[0].imshow(log(file_objects[row].detector.counts+1), extent=extent, cmap = jet, aspect='auto', origin='lower', interpolation='nearest')
    ...
    def ProcessDirectory(self):
        ...
        print data_dir
        file_list = glob1(data_dir, 'I*.c?[1d]')
        if len(file_list) < 1: 
