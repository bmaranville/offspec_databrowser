...
class plot_2d_data(wx.Frame):
    ...
    def __init__(self, data, extent=None, caller = None, scale = 'log', window_title = '2d plot', pixel_mask = None, plot_title = "data plot", x_label = "x", y_label = "y", parent=None):
    ...
    def do_xy_slice(self, x_range, y_range):
        ...
        try:
            avg = sum(data[x_pixel_min:x_pixel_max,y_pixel_min:y_pixel_max]) / sum(normalization_matrix[x_pixel_min:x_pixel_max,y_pixel_min:y_pixel_max])
            print "average pixel value: %g" % avg
