...
def load_plottable_2d_data2(filename=None):
    if filename == None:
        dlg = wx.FileDialog(None, "Load plottable_2d_data object from file:", '', "", "*.*", style=wx.FD_OPEN)
        if dlg.ShowModal() == wx.ID_OK:
            fn = dlg.GetFilename()
        ...
        filename = fd + '/' + fn
    f = open(filename, 'r')
    paramstr = f.readline()[1:-1]
    params = eval(paramstr)
    f.close()
    data = loadtxt(filename, skiprows=2, delimiter='\t')
    initobj = plottable_2d_data(data, params_in=params)
    return initobj
...
class plottable_2d_data:
    ...
    def __getstate__(self):
        ...
        del odict['creator']
        del odict['supervisor']
        del odict['base_data_obj']
        del odict['area_plot']
        return odict
    ...
    def logplot(self, show_sliceplots=True):
        ...
        im = ax.imshow(transformed_show_data, interpolation='nearest', aspect='auto', origin='lower',cmap=jet, extent=(x_min,x_max,y_min,y_max))
        fig.im = im
    ...
    def plot(self, show_sliceplots=True):
        ...
        im = ax.imshow(transformed_show_data, interpolation='nearest', aspect='auto', origin='lower',cmap=hot, extent=(x_min,x_max,y_min,y_max))
        fig.im = im
